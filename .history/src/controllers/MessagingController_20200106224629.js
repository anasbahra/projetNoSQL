...
async function isUserInRoom(userMail , roomId){
    ...
    let roomOwner = await Room.findOne({ userOwner : user._id}).populate('userOwner');
    console.log('room owner : ',rom.userOwner)
    
    let room =  await Room.findOne( { broadcastingUserList: mongoose.Types.ObjectId(user._id) }).populate('broadcastingUserList');
