...
async function isUserInRoom(userMail , roomId){
    ...
    console.log('user found : ',await Room.find({_id:roomId, userOwner: mongoose.Types.ObjectId(user._id)}));
    let room =  await Room.findOne( { _id: roomId, broadcastingUserList: mongoose.Types.ObjectId(user._id) }).populate('broadcastingUserList');
