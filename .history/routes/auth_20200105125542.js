...
router.get('/room',checkAuthenticated, async (req,res) => {
    let roomList = await Room.findOne({ userOwner: req.user._id });
    console.log('roomList',roomList);
    res.render(path.join(__dirname + '/../src/web/Room'),{rooms: roomList == null ? []: rooms })

});

router.post('/room',checkAuthenticated,(req,res) => {
    let room = new Room();
    room.name = req.body.name;
    room.userOwner = req.user._id;
